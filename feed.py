...
def presented_token(request) -> str:
    """The token this request carries — ``Authorization: Bearer`` or ``?token=``.

    Two doors because the puller is somebody else's scheduler: some can
    send a header, some can only be given a URL. The query form is the
    weaker one (a URL lands in access logs and referrers) and it is
    supported anyway, because the alternative for a host whose data manager
    cannot set headers is no feed at all.
    """
    ...
    if header:
        ...
        if scheme.lower() in ("bearer", "token") and value.strip():
            return value.strip()
        return header
    return str(request.GET.get("token") or "").strip()
